...
class Pandoc2Ansi(Pandoc2Plain):
	...
	"""
	def __init__(self, width: int = 80, padding: int = 0, baseColour: tuple[int,
	int, int] = (4, 0, 4)):
		self.content = []
	...
	def catImage(self, content: str):
		...
		try:
			...
			try:
				self.print(
				catimage.generateHDColour(os.getcwd() + os.sep + content, self.width))
	...
	def header(self, content: tuple[int, Any, list[Inline]]):
		...
		if content[0] > 3:
			...
			concatContent = "".join([
			processpandoc.toPlaintext(part) for part in content[2]])
			...
			if content[0] == 1:
				...
				self.print(FG + headColour[content[0]] +
				"\n".join(art.text2art(concatContent, # type: ignore
				"swan").split("\n")[2:-limit - 1]))
			...
			if content[0] == 2:
				...
				self.print(FG + headColour[content[0]] + "\n".join(
				art.text2art(concatContent, "thin").split("\n")[1:-limit])) # type: ignore
				self.print("━" * self.width + CLR)
			if content[0] == 3:
	...
	def codeBlock(self, content: tuple[Any, str]):
		...
		codeColour = self.ansiFromAnsiRgb(
		self.varBaseC(self.baseColour, (-1, -1, -1)))
		self.print(FG + codeColour + "\n │ " + CLR_FG, end="")
		try:
			self.print(FG + codeColour + " │ ".join([
			CLR_FG + str(
	...
	def definitionList(self, content: list[tuple[list[Inline], list[list[Block]]]]):
		...
		for definition in content:
			...
			self.newline()
			self.print(
			FG + self.ansiFromAnsiRgb(self.varBaseC(self.baseColour, (-1, -1, -2))),
			end="")
	...
	def blockQuote(self, content: list[Block]):
		...
		print(
		"\n" + FG + self.ansiFromAnsiRgb(self.varBaseC(self.baseColour,
		(-2, -2, -2))) + " ┃ ", end="")
		print("\n ┃ ".join([
		processpandoc.toPlaintext(block).split("\n")
		for block in content][0]) + CLR_FG)
	...
	def ansiFromAnsiRgb(self, colour: tuple[int, int, int]) -> str:
		...
		return str(
		int(16 + min(max(red, 0), 5) * 36 + min(max(green, 0), 5) * 6 +
		min(max(blue, 0), 5))) + "m"

	def varBaseC(self, base: tuple[int, int, int], modify: tuple[int, int,
